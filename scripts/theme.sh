...
generate_configs() {
  ...
  # sway: wallpaper exec + client colors
  {
    ...
    # Kill any previously-spawned swaybg before starting a new one: exec_always
    # re-runs on every reload/rotation tick but never replaces or reaps the
    # prior instance, so without this every theme change or rotation leaks a
    # swaybg process (observed accumulating to 50+ during normal use).
    if [ "$mode" = "flat" ]; then
      echo "exec_always pkill -x swaybg; swaybg -c \"$BASE\""
    else
      printf 'exec_always pkill -x swaybg; swaybg -i %s -m fill\n' "$(sway_shell_arg "$wallpaper")"
    fi
    echo
