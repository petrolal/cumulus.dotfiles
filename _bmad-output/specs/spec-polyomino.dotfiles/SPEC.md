---
id: SPEC-polyomino.dotfiles
companions:
  - ../../architecture/architecture-polyomino.dotfiles-2026-08-25/ARCHITECTURE-SPINE.md
sources:
  - ../../prds/prd-polyomino.dotfiles-2026-08-25/prd.md
---

> **Canonical contract.** This SPEC and the files in `companions:` are the complete, preservation-validated contract for what to build, test, and validate. Source documents listed in frontmatter are for traceability — consult them only if you need narrative rationale or prose color this contract intentionally omits.

# polyomino.dotfiles Specification
...
- **CAP-1**
  ...
  - **success:** Running `polyomino install` on a bare Ubuntu/Arch VM successfully sets up all required system packages and the CLI without manual intervention.
- **CAP-2**
  ...
  - **success:** A pre-existing `~/.config/sway` is safely moved to `~/.polyomino_backup/<timestamp>/` and a symlink is created in its place.
- **CAP-3**
...
- **CAP-6**
  ...
  - **success:** `polyomino healthcheck` successfully flags broken symlinks or missing fonts without mutating the filesystem.
