...
fn deploy_binaries(ctx: &Context, inst: &Installer) -> Result<()> {
    ...
    if util::command_exists("cargo") {
        ...
        if !inst.dry_run() {
            let cargo_toml = ctx.dotfiles_dir.join("rust/cumulus-dotfiles/Cargo.toml");
            ...
            let release_dir = ctx
                .dotfiles_dir
                .join("rust/cumulus-dotfiles/target/release");
            ...
            println!(
                "+ cargo build --release --manifest-path '{}/rust/cumulus-dotfiles/Cargo.toml'",
                ctx.dotfiles_dir.display()
            );
            println!(
                "+ cp -f '{}/rust/cumulus-dotfiles/target/release/cumulus*' '{}/'",
                ctx.dotfiles_dir.display(),
                bin_dir.display()
            );
        }
